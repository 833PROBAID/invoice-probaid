...
const riskBands = [
	{
		...
		max: 9,
		label: "Low Friction",
		description: "Access is under control. Vendors can mobilize on standard timelines.",
...
const questions = [
	{
		...
		category: "Occupancy",
		options: [
			{ text: "Vacant", points: 1, insight: "Vacant homes enable immediate vendor access." },
			{ text: "Occupied by cooperative family/friends", points: 2, insight: "Track commitments in writing to preserve cooperation." },
			{ text: "Occupied by uncooperative occupants", points: 3, insight: "Engage counsel to formalize notices and expectations." },
		],
	},
	{
		id: "keys",
		...
		category: "Access",
		options: [
			{ text: "Yes, all keys available", points: 1, insight: "Log key transfers and store spares in a lockbox." },
			{ text: "Some keys available, others missing", points: 2, insight: "Schedule a locksmith and document restricted areas." },
			{ text: "No keys available", points: 3, insight: "Coordinate legal entry authority with counsel immediately." },
		],
	},
	{
		id: "cooperation",
		...
		category: "Engagement",
		options: [
			{ text: "Fully cooperative", points: 1, insight: "Keep weekly touchpoints to cement trust." },
			{ text: "Somewhat cooperative", points: 2, insight: "Provide written schedules and confirm receipt." },
			{ text: "Uncooperative or hostile", points: 3, insight: "Use attorney-led communication only." },
		],
	},
	{
		id: "condition",
		...
		category: "Readiness",
		options: [
			{ text: "Clean and well-maintained", points: 1, insight: "Prioritize photography and disclosures." },
			{ text: "Average condition", points: 2, insight: "Budget extra prep days for staging." },
			{ text: "Poor condition, needs significant work", points: 3, insight: "Sequence remediation vendors before marketing." },
		],
	},
	{
		id: "conflict",
		...
		category: "Alignment",
		options: [
			{ text: "No conflict", points: 1, insight: "Share milestone updates bi-weekly to stay aligned." },
			{ text: "Minor disagreements", points: 2, insight: "Clarify decision rights and document consensus." },
			{ text: "Major conflict or disputes", points: 3, insight: "Use mediator or attorney to avoid escalation." },
		],
	},
	{
		id: "security",
		...
		category: "Security",
		options: [
			{ text: "No security issues", points: 1, insight: "Continue property checks twice weekly." },
			{ text: "Minor security concerns", points: 2, insight: "Install temporary cameras or sensors." },
			{ text: "Significant security risks", points: 3, insight: "Engage security patrol until access stabilizes." },
		],
	},
];
...
const OccupantAccessRiskAnalyzer = () => {
	...
	const deriveBand = (score) =>
		riskBands.find((band) => score >= band.min && score <= band.max) || riskBands[0];
					...
					<div className='flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4'>
						<div>
							...
							<p className='text-sm sm:text-base md:text-lg mt-3 max-w-3xl text-white/90'>
								Quantify friction from occupants, keys, and coordination so you can
								stage the property with confidence.
							</p>
						</div>
				...
				<div className='p-4 sm:p-6 space-y-8'>
					<div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
						<div className='bg-[#0097A7]/10 border border-[#0097A7]/20 rounded-xl p-4'>
							...
							<p className='text-xs text-gray-600 mt-2'>
								{results ? results.band.description : "Answer each lever to determine your access posture."}
							</p>
						</div>
						...
						<div className='bg-[#007A87]/10 border border-[#007A87]/20 rounded-xl p-4'>
							...
							<p className='text-xs text-gray-600 mt-2'>
								{results ? `Top delay drivers: ${results.insightPills.frictionDrivers.length || 0}` : "Guidance unlocks when all responses are captured."}
							</p>
						</div>
					</div>
					...
					<div>
						...
						<div className='space-y-6'>
							{questions.map((q, index) => (
								<div key={q.id} className='bg-gray-50 border border-gray-100 p-4 rounded-lg shadow-sm'>
									<div className='flex flex-col md:flex-row md:items-center md:justify-between gap-2'>
										...
										<div className='text-sm font-semibold text-gray-600'>
											{answers[q.id] !== undefined ? q.options[answers[q.id]].text : "Pending"}
										</div>
									</div>
										...
										{q.options.map((option, optionIndex) => (
											<div key={optionIndex} className='w-full'>
												<Checkbox
													...
													onChange={(e) => handleAnswer(q.id, optionIndex, e.target.checked)}
													containerClass='text-gray-700 text-sm sm:text-base leading-relaxed break-words'
													width='full'
												/>
												<p className='text-xs text-gray-500 ml-10'>{option.insight}</p>
											</div>
										))}
									</div>
								</div>
					...
					{results && (
									...
									<div>
										...
										{results.insightPills.frictionDrivers.length ? (
											<ul className='list-disc list-inside text-sm text-gray-700 mt-2 space-y-1'>
												{results.insightPills.frictionDrivers.map((item, index) => (
													<li key={index}>{item}</li>
												))}
											</ul>
										) : (
											<p className='text-sm text-gray-500 mt-2'>No critical drivers flagged.</p>
										)}
									</div>
									<div>
										...
										{results.insightPills.watchList.length ? (
											...
											<p className='text-sm text-gray-500 mt-2'>No watch items currently.</p>
										)}
									</div>
								</div>
								...
								<div className='mt-6'>
									...
									<div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
										{Object.entries(categoryHealth).map(([category, data]) => {
											const percent = 100 - Math.round((data.total / (data.count * 3)) * 100);
											return (
												<div key={category} className='bg-white border border-gray-100 rounded-lg p-4 shadow-sm'>
													<p className='text-xs text-gray-500 uppercase tracking-wide'>
														{category}
													</p>
													<p className='text-2xl font-bold text-[#0097A7]'>{percent}%</p>
													<p className='text-xs text-gray-500'>Stability rating</p>
												</div>
											);
										})}
								</div>
						...
						<p className='text-center text-xs text-gray-500 italic'>
								Disclaimer: Coordinate with your attorney for legal access direction.
							</p>
						</div>
					)}
				</div>
