...
const ExecutorReadinessQuiz = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://invoice.833probate.com/api/quiz/register-user",
        {
          method: "POST",
          headers: {
  ...
  const calculateReadiness = async () => {
    ...
    if (userId) {
      try {
        const response = await fetch(
          "https://invoice.833probate.com/api/quiz/submit",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="bg-gradient-to-r from-[#0097A7] to-[#007A87] text-white p-6 rounded-t-xl">
              <h2 className="text-2xl font-bold">
                Access Executor Readiness Tool
              </h2>
            ...
            <form onSubmit={handleFormSubmit} className="p-6 space-y-4">
              ...
              <p className="text-xs text-gray-500 text-center">
                Your information will be used to provide you with personalized
